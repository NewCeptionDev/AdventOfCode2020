...
public final class Day15 {
  ...
  private static final List<Integer> INPUT = List.of(16, 11, 15, 0, 1, 7);

  private static final int SEARCHED_INDEX_TASK1 = 2020;
  private static final int SEARCHED_INDEX_TASK2 = 30000000;

  public static void main(String[] args) {
    new Day15(INPUT);
  }

  private Day15(List<Integer> inputs) {
  ...
  private int calculateNextNumber(int lastNumber, Map<Integer, Tuple<Integer, Integer>> numberAndLastIndex) {
    ...
    if(numberAndLastIndex.get(lastNumber).getY() == null) {
      ...
      return numberAndLastIndex.get(lastNumber).getX() - numberAndLastIndex.get(lastNumber).getY();
    }
   ...
   */
  private void updateNumberAndLastIndexMap(Map<Integer, Tuple<Integer, Integer>> numberAndLastIndices, int index,
                                           int currentNumber) {
    if(numberAndLastIndices.containsKey(currentNumber)) {
      numberAndLastIndices.put(currentNumber, new Tuple<>(index, numberAndLastIndices.get(currentNumber).getX()));
    } else {
