...
public class Day10 {
  ...
  public static final String FILE_NAME = "day10Task1Input";
  ...
  private Day10(List<String> inputs) {
  ...
  public long task2(final List<String> inputs) {
    ...
    return splitByThreeJoltJumps.stream().map(this::returnDistinctArrangements).filter(integer -> integer != 0)
                                .map(integer -> (long) integer).reduce(1L, (integer, integer2) -> integer * integer2);
  }

  /**
   * Splits the List of Integer into multiple Lists Integer are split when the Difference between two Integer is 3
   *
