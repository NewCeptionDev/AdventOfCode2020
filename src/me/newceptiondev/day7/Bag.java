...
public class Bag {
  ...
  public Bag(final String code) {
    ...
    color = split[0].substring(0, split[0].indexOf("bags") - 1);
  ...
  private void parseContainedBags(final String[] containedBags) {
    for(String bag : containedBags) {
      ...
      if(!trimmedBag.contains("no other bags.")) {
        int count = Integer.parseInt(trimmedBag.substring(0, 1));
        String color = trimmedBag.substring(2, trimmedBag.indexOf("bag") - 1);
