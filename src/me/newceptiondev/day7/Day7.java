...
public class Day7 {
  ...
  private static final String FILE_NAME = "day7Task1Input";
  ...
  private Day7(List<String> inputs) {
   ...
   *
   * @return Count of Bag Colors that can possible contain the searched colored Bag
   */
  public int task1(final List<String> inputs) {
    ...
    Set<String> colors = getColorsOfBagsContainingColor(SEARCHED_COLOR, new HashSet<>(), bagWithTheirParentBags);
  ...
  private Map<String, Set<String>> getColorsAndBagsContainingThem(final List<Bag> bags) {
    ...
    for(Bag bag : bags) {
      getBagColors(bagsWithParents, bag);
    }
  ...
  /**
   * Add all Colors to the given Map
   *
   * @param bagsWithParents Map with Bag and Set of Colors
   * @param bag             Bag
   */
  private void getBagColors(final Map<String, Set<String>> bagsWithParents, final Bag bag) {
  ...
  /**
   * Returns all Colors of Bags that contain the given Color Recursively called
   *
   ...
   */
  private Set<String> getColorsOfBagsContainingColor(final String color, Set<String> alreadyChecked,
                                                     final Map<String, Set<String>> mappedColors) {
    ...
    if(mappedColors.containsKey(color)) {
      for(String parent : mappedColors.get(color)) {
        if(!alreadyChecked.contains(parent)) {
          ...
          parents.add(parent);
          parents.addAll(getColorsOfBagsContainingColor(parent, alreadyChecked, mappedColors));
        }
      }
    }
  ...
  /**
   * Returns the Count of Bags needed within a Bag of a given Color Recursively
   *
  ...
  private int getNeededBags(final String color, final Map<String, Bag> colorBagMap) {
    ...
    for(String neededColor : bag.getContains().keySet()) {
      neededBags += bag.getContains().get(neededColor) * getNeededBags(neededColor, colorBagMap);
    }
