...
public class Day14 {
  ...
  private static final String FILE_NAME = "day14Task1Input";
  private static final int ARRAY_LENGTH = 36;
  ...
  private Day14(List<String> inputs) {
  ...
  public long task1(List<String> inputs) {
    Integer[] bitmask = new Integer[ARRAY_LENGTH];
    ...
    for(String input : inputs) {
      if(input.startsWith("mask")) {
        ...
        Tuple<Integer, Integer[]> memoryAddressAndValue = applyMaskToValueForTask1(bitmask, input);

        memory.put(memoryAddressAndValue.getX(), memoryAddressAndValue.getY());
      }
    }

    Optional<Long> valueSum = memory.values().stream().map(this::convertBitArrayToLong).reduce(Long::sum);

    if(valueSum.isPresent()) {
      return valueSum.get();
    }

    System.err.println("There was an Error summing up all Values");
    return 0L;
  }
  ...
  public long task2(List<String> inputs) {
    ...
    for(String input : inputs) {
      if(input.startsWith("mask")) {
        ...
        Tuple<List<Integer[]>, Long> memoryAddressAndValue = applyMaskToValueForTask2(bitmask, input);

        for(int i = 0; i < memoryAddressAndValue.getX().size(); i++) {
          memory.put(convertBitArrayToLong(memoryAddressAndValue.getX().get(i)), memoryAddressAndValue.getY());
        }
      }
    }

    Optional<Long> valueSum = memory.values().stream().reduce(Long::sum);
  ...
  /**
   * Maps a char[] to an Integer[] Can fill unused Elements with 0 or null
   *
  ...
  private Integer[] mapCharsToInt(char[] chars, boolean fillWithZero) {
    ...
    for(int i = ARRAY_LENGTH - 1; i >= 0; i--) {
      ...
      if(chars.length >= realIndex &&
         (chars[chars.length - realIndex] == '0' || chars[chars.length - realIndex] == '1')) {
  ...
  private Tuple<List<Integer[]>, Long> applyMaskToValueForTask2(Integer[] bitmask, String value) {
    ...
    return new Tuple<>(generatePossibleMemoryAddress(bitmaskApplied), memoryAddressAndParsedValue.getY());
  }
  ...
  private Tuple<Integer, Integer[]> parseMemoryValue(String input) {
    ...
    int memoryAddress = Integer.parseInt(input.substring(input.indexOf("[") + 1, input.indexOf("]")));
    long memoryValue = Long.parseLong(input.substring(input.indexOf("=") + 2));
    String binaryValue = Long.toBinaryString(memoryValue);

    return new Tuple<>(memoryAddress, mapCharsToInt(binaryValue.toCharArray(), true));
  }
  ...
  private Tuple<Integer[], Long> parseMemoryAddress(String input) {
    ...
    return new Tuple<>(mapCharsToInt(binaryAddress.toCharArray(), true), memoryValue);
  }
  ...
  private Long convertBitArrayToLong(Integer[] bits) {
    Optional<String> bitsRepresentation = Arrays.stream(bits).map(String::valueOf).reduce(String::concat);

    if(bitsRepresentation.isPresent()) {
      return Long.parseLong(bitsRepresentation.get(), 2);
    }

    System.err.println("There was an Error connecting the Bits");
    return 0L;
  }
  ...
  /**
   * Builds a changed Version of a given Integer[]
   *
